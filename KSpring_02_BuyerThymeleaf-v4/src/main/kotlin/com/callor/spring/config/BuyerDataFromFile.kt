...
class BuyerDataFromFile (val buyerDao: BuyerRepository){
    ...
    fun dataFromFile():CommandLineRunner {
        ...
        /**
         * dataFile 변수를 null 허용 변수로 만들었기 때문에
         * 다른 변수에 담을때는 아래와 같은 키워드를 사용한다
         * dataFile 변수에 담긴 값을 {} 부분으로 전달(let, 할당)하여
         ...
         * 다음과 같은 속성값을 취할 수 있다
         */
        logger().debug("파일 객체 {} ", file?.file.toString())
        logger().debug("파일 이름 {} ", file?.filename.toString())
        logger().debug("InputStream {} ", file?.inputStream.toString())
        logger().debug("파일 저장 경로 객체 {} ", file?.path.toString())
        logger().debug("URL {} ", file?.url.toString())
        logger().debug("URI {} ", file?.uri.toString())
         ...
         * text 파일의 전체 내용을 한 라인씩 잘라서 List Data로 생성해 준다
         */
        ...
        val managerList = fileList.map {
            val lines = it.split(":")
            // {이름 : "홍길동", 전화번호:"010-1111-1111"} 형식의
            // 데이터 생성
            mapOf(이름 to lines[이름], 전화번호 to lines[전화번호])
        }
        val buyerList = fileList.mapIndexed {index, str:String ->
            val lines = str.split(":")
